...
image: /images/distributed-improvement/images.png
subtitle: Techniques for faster and more stable distributed training
author: hoangbm
...
$$ w_{t+k} = w_t - \eta\frac{1}{n} \sum_{j<k} \sum_{x\in\beta_j} \bigtriangledown l(x, w_t+j)$$

On the other hand, taking a single step with a large mini-batch and learning rate $$\hat\eta$$ will result in:  
$$ \hat w_{t+1} = w_t - \hat\eta\frac{1}{kn} \sum_{j<k} \sum_{x\in\beta_j} \bigtriangledown l(x, w_t)$$
